...
public class TileEntityFusionAltar extends TileEntity implements ITickable
{
    ...
    public static boolean isBurning(TileEntityFusionAltar te) {
    ...
    public void update() {
        if(this.isBurning()) {
            --this.burnTime;
            BlockFusionAltar.setState(true, world, pos);
        }
        ...
        if(this.isBurning() && this.canSmelt() && cookTime > 0) {
            ...
            if(this.canSmelt() && this.isBurning()) {
                ItemStack output = FusionAltarRecipes.getInstance().getFusionResult(inputs[0], inputs[1]);
                if(!output.isEmpty()) {
                    smelting = output;
                    cookTime++;
                    inputs[0].shrink(1);
    ...
    private boolean canSmelt() {
        if(((ItemStack)this.handler.getStackInSlot(0)).isEmpty() || ((ItemStack)this.handler.getStackInSlot(1)).isEmpty()) return false;
            ...
            ItemStack result = FusionAltarRecipes.getInstance().getFusionResult((ItemStack)this.handler.getStackInSlot(0), (ItemStack)this.handler.getStackInSlot(1));
