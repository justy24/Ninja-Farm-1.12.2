...
public class ContainerFusionAltar extends Container {
    private final TileEntityFusionAltar tileentity;
    private int cookTime, totalCookTime, burnTime, currentBurnTime;
    private ItemStackHandler handler = new ItemStackHandler(4);

    public ContainerFusionAltar(InventoryPlayer player, TileEntityFusionAltar tileentity)
    {
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if(slot != null && slot.getHasStack())
        {
            ...
            else if(index != 2 && index != 1 && index != 0)
            {
                ...
                if(!FusionAltarRecipes.getInstance().getFusionResult(stack1, slot1.getStack()).isEmpty())
                {
                    if(!this.mergeItemStack(stack1, 0, 2, false))
                    {
                        return ItemStack.EMPTY;
                    }
                    else if(TileEntityFusionAltar.isItemFuel(stack1))
                    {
